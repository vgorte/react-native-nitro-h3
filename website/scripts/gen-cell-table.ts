...
type Bounds = { qMin: number; qMax: number; rMin: number; rMax: number }
 ...
 */
function reachableBounds(boxes: readonly Point[]): Bounds {
  const s = SIZES[RES]
 ...
 */
function buildTable(bounds: Bounds): Table {
  const s = SIZES[RES]
  const ids: string[] = []
  const codes: number[] = []
...
function render(table: Table): string {
  const { bounds } = table
  const lines = [
