...
The authoritative mapping from this package's exported surface to the H3 C library it binds.
Every row is read off the upstream header named under Provenance.
...
| Maintenance | Rebuilt against the pinned upstream when the vendored H3 version changes, never edited row by row. |
| Known limitation | Line numbers and `@brief` quotes are read by hand from the pinned header, so an upstream edit that keeps the tag but moves a line silently ages them; nothing in CI re-reads this page. |
| Cutoff | 2026-09-11, against `v4.5.0`. |
...
| `gridDistance` | `gridDistance` | `H3Error gridDistance(H3Index origin, H3Index h3, int64_t *distance);` | S9 | n/a | `int64_t*` (named `distance`), no sentinel | H3Error | header line 821. S9's out type varies (`int` vs `H3Index` vs `int64_t`); template must be parameterised on it. |
...
| `getIcosahedronFaces` | `getIcosahedronFaces` | `H3Error getIcosahedronFaces(H3Index h3, int *out);` | S13 | `maxFaceCount` | `int*`, **-1 padded** (not H3_NULL) | H3Error | header line 691. `h3Index.c:1238-1244`: "The array is sparse; since 0 is a valid value, invalid array values are represented as -1. It is the responsibility of the caller to filter out invalid values." and "@param out Output array. Must be of size maxFaceCount(h3)." This is the row that forces S13 to be parameterised on element type + sentinel predicate. |
...
| `gridRingUnsafe` | `gridRingUnsafe` | `H3Error gridRingUnsafe(H3Index origin, int k, H3Index *out);` | S14 | `maxGridRingSize` | `H3Index*`, partial-write on failure, so **discard, do not compact** | H3Error | header line 302. Returns `E_PENTAGON` after having already written part of the buffer. |
...
| `childPosToCell` | `childPosToCell` | `H3Error childPosToCell(int64_t childPos, H3Index parent, int childRes, H3Index *child);` | ONE-OFF | n/a | `H3Index*` (named `child`), no sentinel | H3Error | header line 636. Three inputs `(int64_t, H3Index, int)` match no shape in this taxonomy: S7 is `(H3Index, int)`. `h3-js` signature is `childPosToCell(childPos, h3Index, childRes)`, the same order as C. |
...
| `greatCircleDistanceKm` | `greatCircleDistanceKm` | `double greatCircleDistanceKm(const LatLng *a, const LatLng *b);` | ONE-OFF | n/a | none (scalar return) | **none** | header line 377. Two struct inputs, `double` return, no error channel at all. The TS surface takes two `[lat, lng]` pairs as in `h3-js` and unpacks them into four doubles at the binding. |
| `greatCircleDistanceM` | `greatCircleDistanceM` | `double greatCircleDistanceM(const LatLng *a, const LatLng *b);` | ONE-OFF | n/a | none (scalar return) | **none** | header line 381. The TS surface takes two `[lat, lng]` pairs as in `h3-js`. |
| `greatCircleDistanceRads` | `greatCircleDistanceRads` | `double greatCircleDistanceRads(const LatLng *a, const LatLng *b);` | ONE-OFF | n/a | none (scalar return) | **none** | header line 372. The TS surface takes two `[lat, lng]` pairs as in `h3-js`. |
...
The remaining **54** are all covered by the 64-row table (the 5 unit-dispatch operations expand to 13 rows).
**No user-facing `h3-js` operation is missing.**
Conversely this package adds two operations `h3-js` does not have ([`cellToString`](../packages/react-native-nitro-h3/docs/api.md#celltostring)/`h3ToString`, [`cellFromString`](../packages/react-native-nitro-h3/docs/api.md#cellfromstring)/`stringToH3`), because `h3-js` represents indexes as hex strings natively and therefore needs no conversion functions.
...
Three fixed-N buffers have no size function.
Their sizes are documented only in prose, and for two of the three not even in the header.
...
The header gives no number.
The only place in the C library that states it is the implementation:
...
`CellBuffer` must therefore value-initialize (`calloc` / `std::vector<H3Index>(n)`), never `malloc`/`reserve`.
Uninitialised memory would be read back as bogus cells after the H3_NULL compaction pass, silently.
...
Both take the same `const GeoPolygon *`.
The `GeoLoop::verts` arrays it points at must stay alive across both calls.
Same for the experimental pair (lines 329-335).
...
All other size sources (`maxGridDiskSize`, `maxGridRingSize`, `maxPolygonToCellsSize`, `maxPolygonToCellsSizeExperimental`, `uncompactCellsSize`, `gridPathCellsSize`, `maxFaceCount`, `cellToChildrenSize`) return `H3Error` and must be checked before the allocation.
...
The header does not say what `mode` may be.
...
Every row here is read off the pinned upstream header, by hand.
Open an issue if a signature, a line number or a size source is wrong.
